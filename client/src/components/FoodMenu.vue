        ...
        <div class="row">
          ...
          <div class="col-9" v-if="searchedFoods == null">
            <ul class="list-group list-group-flush">
              ...
              <div class="items">
                <div class="list-group-item list-group-item-action flex-column align-items-start" v-for="food in foods" :key="food.id">
                  <div class="d-flex w-100 justify-content-between">
                    <h6 class="mb-1">{{ food.title }}</h6>
                    <small>{{ food.price }}</small>
                    <button class="badge badge-dark badge-pill btn" title="Add to Order list" @click="addOrder(food)">+</button>
                  </div>
          ...
          <div class="col-9" v-else>
            <ul class="list-group list-group-flush">
              <li class="list-group-item d-flex justify-content-between align-items-center bg-light">
                ...
                <span class="badge badge-primary badge-pill">{{ searchedFoods.length }}</span>
              </li>
              <div class="items">
                <div class="list-group-item list-group-item-action flex-column align-items-start" v-for="food in searchedFoods" :key="food.id">
                  <div class="d-flex w-100 justify-content-between">
                    <h6 class="mb-1">{{ food.title }}</h6>
                    <small>{{ food.price }}</small>
                    <button class="badge badge-dark badge-pill btn" title="Add to Order list" @click="addOrder(food)">+</button>
          ...
          <div class="col-3">
            <ul class="list-group list-group-flush">
              <li class="list-group-item d-flex justify-content-between align-items-center bg-light">
                ...
                <button class="btn badge badge-danger" @click="removeOrders"  v-if="orders.length">Remove All Order</button>
              </li>
              <div class="order-list">
                <li class="list-group-item d-flex justify-content-between align-items-center" v-for="order in orders" :key="order.details.id">
                  {{ order.details.title }}
                  <span class="badge badge-primary badge-pill"> {{ order.quantity }}</span>
                  <button class="badge badge-dark badge-pill btn ml-2" title="Remove" @click="removeOrder(order.details)">-</button>
                </li>
              </div>
              <li class="list-group-item d-flex justify-content-between align-items-center bg-light">
                <span>Total</span>
  ...
  methods: {
    addOrder (food) {
      ...
      if (orderIndex === -1) {
        ...
        if (this.orders[orderIndex].quantity < this.orders[orderIndex].details.quantity) {
    ...
    async placeOrder () {
      if (this.orders.length) {
        ...
        // place the order
        try {
          const order = (await OrderService.placeOrder(foodCart)).data;
          if(order.message) {
            this.foods = (await FoodService.foods()).data
            this.orders = [];
            this.$notify({
